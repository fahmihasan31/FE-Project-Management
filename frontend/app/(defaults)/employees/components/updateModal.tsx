...
import { putEmployees } from '@/store/slices/employees';
...
const UpdateModal = ({ isOpen, onClose, employee }: UpdateModalProps) => {
    ...
    const [data, setData] = useState({
        name: '',
        latest_education: '',
        skills: [] as { id: string; rate: number }[],
    });
            ...
            if (employee) {
                setData({
                    ...
                    skills: employee.skills.map((skill) => ({
                        id: skill.master_skill?.id.toString() || '',
                        rate: skill.rate,
                    })),
                });
            }
        }
    }, [dispatch, employee, isOpen]);
        ...
        setData((prevData) => {
            const updatedSkills = prevData.skills.map((skill, i) => {
                if (i === index) {
                    return {
                        ...skill,
                        [name === 'rate' ? 'rate' : 'id']: name === 'rate' ? parseInt(value, 10) : value,
                    };
                }
                return skill;
            });
    ...
    // Get the list of selected skills to disable them in dropdown
    const selectedSkills = data.skills.map((skill) => skill.id);

    const handleAddSkill = () => {
        setData((prevData) => ({
            ...prevData,
            skills: [...prevData.skills, { id: '', rate: 1 }],
        }));
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            await dispatch(putEmployees({ payload: payload, id: employee.id }))
                .unwrap()
                .then(() => {
                    dispatch(getSkills({}));
